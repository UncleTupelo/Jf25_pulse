...
class ScreenshotProcessor(BaseContextProcessor):
    ...
    def _is_duplicate(self, new_context: RawContextProperties) -> bool:
        ...
        # Check for duplicates without removing during iteration
        found_duplicate_idx = None
        for idx, item in enumerate(self._current_screenshot):
            diff = bin(int(str(new_phash), 16) ^ int(str(item["phash"]), 16)).count("1")
            if diff <= self._similarity_hash_threshold:
                found_duplicate_idx = idx
                break

        if found_duplicate_idx is not None:
            # Move duplicate to end (most recently used) by rotating
            # This is more efficient than remove + append for deque
            item = self._current_screenshot[found_duplicate_idx]
            # Remove from current position and append to end
            del self._current_screenshot[found_duplicate_idx]
    ...
    async def _merge_items_with_llm(
        ...
        if not response:
            raise ValueError(
                f"Empty LLM response when merge items for context type: {context_type.value}"
            )
        ...
        if not isinstance(response_data, dict) or "items" not in response_data:
            ...
            raise ValueError(
                f"Invalid response format when merge items for context type: {context_type.value}"
            )
