...
async def trigger_sync(
    ...
    try:
        ...
        if not component.is_running():
            raise HTTPException(
                status_code=400, detail=f"Integration '{integration_name}' is not running"
            )
        ...
        return convert_resp(code=500, status=500, message=f"Sync failed: {str(e)}")
...
async def upload_file(
    ...
    try:
        ...
        if file_upload_name not in capture_manager._components:
            raise HTTPException(status_code=503, detail="File upload integration not configured")
        ...
        if context is None:
            raise HTTPException(
                status_code=400, detail="File upload failed - check file type and size"
            )
        ...
        return convert_resp(code=500, status=500, message=f"Upload failed: {str(e)}")
...
async def start_integration(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail=f"Failed to start integration '{integration_name}'"
            )
        ...
        return convert_resp(code=500, status=500, message=f"Failed to start integration: {str(e)}")
...
async def stop_integration(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail=f"Failed to stop integration '{integration_name}'"
            )
        ...
        return convert_resp(code=500, status=500, message=f"Failed to stop integration: {str(e)}")
